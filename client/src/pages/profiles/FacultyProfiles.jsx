...
const FacultyProfiles = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [profilesRes, usersRes] = await Promise.all([
        api.get("/super-admin/profiles/faculty"),
        api.get("/super-admin/profiles/users/role/FACULTY")
      ]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (editingId) {
        await api.patch(`/super-admin/profiles/faculty/${editingId}`, payload);
        setSuccess("Profile updated successfully.");
        setEditingId(null);
      } else {
        await api.post("/super-admin/profiles/faculty", payload);
        setSuccess("Profile created successfully.");
      }
      setForm({ userId: "", employeeId: "", designation: "", department: "", qualification: "", specialization: "", experience: "", joiningDate: "", salary: "" });
      await fetchData();
    } catch (err) {
  ...
  const deleteProfile = async (profileId) => {
    ...
    try {
      await api.delete(`/super-admin/profiles/faculty/${profileId}`);
  ...
  return (
    <div className="space-y-8">
      <div className="rounded-[32px] border border-slate-800 bg-slate-900/80 p-8 shadow-2xl shadow-slate-950/40">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-semibold text-white">Faculty Profiles</h2>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-8 grid gap-4 xl:grid-cols-[0.7fr_0.3fr]">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="space-y-2 text-sm text-slate-300">
              Employee ID
              <input name="employeeId" value={form.employeeId} onChange={handleInput} required placeholder="FAC-001" className="w-full rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none focus:border-violet-400" />
            </label>
            <label className="space-y-2 text-sm text-slate-300">
              Designation
              <input name="designation" value={form.designation} onChange={handleInput} placeholder="Professor" className="w-full rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none focus:border-violet-400" />
            </label>
            <label className="space-y-2 text-sm text-slate-300">
              Department
              <input name="department" value={form.department} onChange={handleInput} placeholder="Mathematics" className="w-full rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none focus:border-violet-400" />
            </label>
            <label className="space-y-2 text-sm text-slate-300">
              Qualification
              <input name="qualification" value={form.qualification} onChange={handleInput} placeholder="PhD" className="w-full rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none focus:border-violet-400" />
            </label>
            <label className="space-y-2 text-sm text-slate-300">
              Specialization
              <input name="specialization" value={form.specialization} onChange={handleInput} placeholder="Algebra" className="w-full rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none focus:border-violet-400" />
            </label>
            <label className="space-y-2 text-sm text-slate-300">
              Experience
              <input name="experience" value={form.experience} onChange={handleInput} placeholder="10 Years" className="w-full rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none focus:border-violet-400" />
            </label>
            <label className="space-y-2 text-sm text-slate-300">
              Joining Date
            ...
            <label className="space-y-2 text-sm text-slate-300">
              Salary
              <input type="number" name="salary" value={form.salary} onChange={handleInput} placeholder="80000" className="w-full rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none focus:border-violet-400" />
            </label>
          </div>
      ...
      <div className="rounded-[32px] border border-slate-800 bg-slate-900/80 p-8 shadow-2xl shadow-slate-950/40">
        <h3 className="text-2xl font-semibold text-white mb-6">Faculty Directory</h3>
        <TableToolbar
              ...
              <tbody className="divide-y divide-slate-800">
                {filteredProfiles.map(p => (
                  <tr key={p._id} className="border-b border-slate-800">
                    ...
                    <td className="px-4 py-4">{p.specialization || "-"}</td>
