...
const AccountantProfiles = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [profilesRes, usersRes] = await Promise.all([
        api.get("/super-admin/profiles/accountants"),
        api.get("/super-admin/profiles/users/role/ACCOUNTANT")
      ]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (editingId) {
        await api.patch(`/super-admin/profiles/accountants/${editingId}`, payload);
        setSuccess("Profile updated successfully.");
        setEditingId(null);
      } else {
        await api.post("/super-admin/profiles/accountants", { userId: form.userId, ...payload });
        setSuccess("Profile created successfully.");
      }
      setForm({ userId: "", employeeId: "", designation: "", department: "", qualification: "", experience: "", joiningDate: "", salary: "" });
      await fetchData();
    } catch (err) {
  ...
  const deleteProfile = async (profileId) => {
    ...
    try {
      await api.delete(`/super-admin/profiles/accountants/${profileId}`);
  ...
  return (
    <div className="space-y-8">
      <div className="rounded-[32px] border border-slate-800 bg-slate-900/80 p-8 shadow-2xl shadow-slate-950/40">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-semibold text-white">Accountant Profiles</h2>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-8 grid gap-4 xl:grid-cols-[0.7fr_0.3fr]">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="space-y-2 text-sm text-slate-300">
              Employee ID
              <input name="employeeId" value={form.employeeId} onChange={handleInput} required placeholder="ACC-001" className="w-full rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none focus:border-violet-400" />
            </label>
            <label className="space-y-2 text-sm text-slate-300">
              Designation
              <input name="designation" value={form.designation} onChange={handleInput} placeholder="Senior Accountant" className="w-full rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none focus:border-violet-400" />
            </label>
            <label className="space-y-2 text-sm text-slate-300">
              Department
              <input name="department" value={form.department} onChange={handleInput} placeholder="Finance" className="w-full rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none focus:border-violet-400" />
            </label>
            <label className="space-y-2 text-sm text-slate-300">
              Qualification
              <input name="qualification" value={form.qualification} onChange={handleInput} placeholder="MBA Finance" className="w-full rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none focus:border-violet-400" />
            </label>
            <label className="space-y-2 text-sm text-slate-300">
              Experience
          ...
          <div className="rounded-3xl border border-slate-800 bg-slate-950/80 p-6">
            ...
            {!editingId && availableUsers.length === 0 && <p className="mt-4 text-sm text-slate-400">All Accountant users already have profiles.</p>}
      ...
      <div className="rounded-[32px] border border-slate-800 bg-slate-900/80 p-8 shadow-2xl shadow-slate-950/40">
        <h3 className="text-2xl font-semibold text-white mb-6">Accountant Directory</h3>
        <TableToolbar
          ...
          placeholder="Search accountant profiles..."
        />
...
export default AccountantProfiles;
